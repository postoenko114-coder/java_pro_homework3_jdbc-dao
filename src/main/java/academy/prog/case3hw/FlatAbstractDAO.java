...
public abstract class FlatAbstractDAO<T> {
    private final Connection conn;
    private final String table;

    public FlatAbstractDAO(Connection conn, String table) {
        this.conn = conn;
        this.table = table;
    }
    ...
    public List<T> getListByRange(double min, double max, Class<T> cls) {
        List<T> res = new ArrayList<>();

        try {
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("SELECT * FROM " + table + " WHERE price BETWEEN " + min + " AND " + max)) {
                    ResultSetMetaData md = rs.getMetaData();
    ...
    public List<T> getListByArea(double minArea, Class<T> cls) {
        List<T> res = new ArrayList<>();
        try {
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("SELECT * FROM " + table + " WHERE area > " + minArea  )) {
                    ResultSetMetaData md = rs.getMetaData();
